...
void File_v_SafeDatOpen(char *pstr_dat_name, char *pstr_dat_mode)
{
...
void File_v_ReadFromDat(uint8 *pui8_readDat)
{
	char c;
	...
	if (pFile_sudokuFile)
	{
		...
		while ((c = fgetc(pFile_sudokuFile)) != EOF)
		{
			...
			for (i = 0; i < SUDOKU_NUM; i++)
			{
				...
				while ((c = fgetc(pFile_sudokuFile)) != '\n')
				{
					p_st_Element->sudokuTable[i][j] = c - 48; //Substract 48 from ASCII (ASCII 49 - dec 1)
					j++;
				}
			}
			...
			{
				List_v_AddToEndOfList(p_st_Element);
			}
		}
	}
	else
